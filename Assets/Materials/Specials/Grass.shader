Shader "Perso/Grass"
{
	Properties
	{
		...
		[Header(Geometry)]
		[Toggle] _ComplexBlade("Complex mesh for blades", Int) = 0
		...
		void geo(triangle vertexOutput IN[3] : SV_POSITION, inout TriangleStream<geometryOutput> output)
		{
			...
			if (dist < 5)
			{
				for (int i = 0; i < BLADE_SEGMENTS; i++)
				{
					...
					output.Append(GenerateGrassVertex(pos, segmentWidth, segmentHeight, segmentForward, float2(0, t), transformMatrix));
					output.Append(GenerateGrassVertex(pos, -segmentWidth, segmentHeight, segmentForward, float2(1, t), transformMatrix));
				}
				output.Append(GenerateGrassVertex(pos, 0, height, forward, float2(0.5, 1), transformationMatrix));
			}
			else
			{
				output.Append(GenerateGrassVertex(pos, width, 0, 0, float2(0, 0), transformationMatrixFacing));
				output.Append(GenerateGrassVertex(pos, -width, 0, 0, float2(1, 0), transformationMatrixFacing));
				output.Append(GenerateGrassVertex(pos, 0, height, forward, float2(0.5, 1), transformationMatrix));
			}
		}
			ENDCG
